...
from AB3DMOT_Tracker.bbox_utils import convert_3dbox_to_8corner, iou3d
from AB3DMOT_Tracker.kalman_filter import KalmanBoxTracker
...
class AB3DMOT(object):			  # A baseline of 3D multi-object tracking using Hungarain + Kalman filter
	...
	def update(self, dets):
		...
		for trk in reversed(self.trackers):
			...
			if (trk.time_since_update >= self.max_age): 
				self.trackers.pop(i)
		if (len(ret) > 0): return np.concatenate(ret)			# h,w,l,x,y,z,theta, ID, confidence
